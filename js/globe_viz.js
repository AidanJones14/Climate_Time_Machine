...
(function () {
    ...
    // Atmosphere color — visible glow ring around the globe (smoggy at high values)
    const atmColorScale = d3.scaleLinear()
        .domain([-0.5, 0, 0.3, 0.8, 1.2])
        .range(["#1a4a7a", "#3a5a6a", "#a04030", "#b32210", "#881100"])
        .clamp(true);

    // CO₂ → atmosphere thickness (haziness)
    const co2AltitudeScale = d3.scaleLinear()
        .domain([315, 370, 425])
        .range([0.12, 0.20, 0.45])
        .clamp(true);
    ...
    window.addEventListener("yearchange", (e) => {
        ...
        if (co2 !== undefined) {
            ...
            globe.atmosphereAltitude(Math.min(co2Alt + tempAlt, 0.55));
        } else {
